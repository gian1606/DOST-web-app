  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-bold text-text-primary" style={{ fontSize: 28 }}>Barangay Leaderboard</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-full px-4 py-2"
            ...
            <span className="font-semibold" style={{ fontSize: 13, color: "#2E7D32" }}>Batangas City</span>
          </div>
          <div className="flex items-center gap-2 rounded-full px-4 py-2"
      ...
      <div className="bg-white rounded-xl px-4 py-3 flex items-center gap-3"
        ...
        <label className="font-medium text-text-secondary" style={{ fontSize: 13 }} htmlFor="month-select">
          Month:
        </label>
        <select id="month-select" value={month} onChange={(e) => setMonth(e.target.value)}
          className="rounded-lg px-3 py-2 outline-none"
          style={{ fontSize: 13, border: "1.5px solid #E5E7EB", background: "#F9FAFB", color: "#1A1A1A", minWidth: 180 }}>
        ...
        <span className="ml-auto text-text-muted" style={{ fontSize: 12 }}>{selectedLabel}</span>
      </div>
      ...
      {topThree.length >= 1 && (
        <div className="bg-white rounded-xl p-6"
          ...
          <div className="flex items-end gap-2">
            {/* 2nd place */}
            <div className="flex-1 flex flex-col items-center justify-between rounded-t-xl px-3 py-5 overflow-hidden"
      ...
      <div className="bg-white rounded-xl overflow-hidden"
        ...
        <div className="px-5 py-3 border-b flex items-center justify-between"
          ...
          <span className="text-text-muted" style={{ fontSize: 12 }}>{selectedLabel}</span>
        </div>

        <table className="w-full border-collapse">
          <thead>
            <tr style={{ background: "#F9FAFB", borderBottom: "1px solid #E5E7EB" }}>
              {["Rank", "Barangay", "Reports", "Eco Points", "Collection Rate", "Badge"].map((h) => (
          ...
          <tbody>
            {ranked.length === 0 ? (
              ...
              return (
                <tr key={r.barangayId}
                  style={{
                    background: r.displayRank <= 3 ? (r.displayRank === 1 ? "#FFFBEB" : "#FAFAFA") : i % 2 === 0 ? "#fff" : "#FAFAFA",
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-text-primary" style={{ fontSize: 13 }}>{r.name}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-1.5">
                      <Star size={13} color="#D97706" />
                      <span className="font-semibold text-text-primary" style={{ fontSize: 13 }}>{r.reportsSubmitted}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3">
                    <span className="font-bold text-primary" style={{ fontSize: 14 }}>{r.pointsEarned.toLocaleString()}</span>
                    <span className="text-text-muted ml-1" style={{ fontSize: 12 }}>pts</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className="font-semibold" style={{ fontSize: 13, color: r.collectionRate >= 90 ? "#2E7D32" : r.collectionRate >= 80 ? "#D97706" : "#DC2626" }}>
                      {r.collectionRate}%
                    </span>
                  </td>
                  <td className="px-5 py-3">
